...
class _BodyState extends State<Body> {
  ...
  //Listas de workshop
  List<SesionGrupal> listSesionGrupal;
  List<SesionGrupal>
      originalListSesionGrupal; //para mostrar todos los workshops
  ...
  void initState() {
    super.initState();
    this.listSesionGrupal = SesionGrupalService.listaSesionGrupal;
    this.originalListSesionGrupal = SesionGrupalService.listaSesionGrupal;
  ...
  filterByDate(DateTime selectDate) {
    setState(
      () {
        this.listSesionGrupal = [];
      },
    );
    if (this.originalListSesionGrupal.isEmpty == false) {
      for (int i = 0; i < this.originalListSesionGrupal.length; i++) {
        String dateTime = originalListSesionGrupal[i].hour;
        ...
        if (resultComparate == true) {
          setState(
            () {
              this.listSesionGrupal.add(originalListSesionGrupal[i]);
            },
          );
        }
      }
    ...
    setState(
      () {
        ...
        if (stateFilter == false) {
          ...
          this.textFilter = "Filtrar por:";
          this.iconFilter = Icons.calendar_today;
          this.listSesionGrupal = this.originalListSesionGrupal;
        }
      },
    );
  }
  ...
  showListSesionGrupal() {
    List<Widget> widgets = [];
    Size size = MediaQuery.of(context).size;
    if (this.listSesionGrupal.isEmpty == false) {
      //Para mostra lista de workshop
      for (int i = 0; i < this.listSesionGrupal.length; i++) {
        String date = getDatefromDateTime(listSesionGrupal[i].hour);
        String time = getTimeFromDateTime(listSesionGrupal[i].hour);
        String dateInvert = invertDate(date);
        if (this.listSesionGrupal[i].totalCapacity >
            this.listSesionGrupal[i].participants.length) {
          widgets.add(
            GestureDetector(
              onTap: () {
                SesionGrupalService.setSesionGrupal(this.listSesionGrupal[i]);
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      width: 90,
                      child: Image.asset(
                        this.listSesionGrupal[i].especialista.image,
                        scale: 1,
                      ),
                    ),
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            child: Text(
                              "Especialista: ${this.listSesionGrupal[i].especialista.name}",
                              style: TextStyle(
                                color: Color(0xffababab),
                                fontSize: 13,
                                fontFamily: 'Raleway',
